...
import nodomain.freeyourgadget.gadgetbridge.RateService;
...
//            return true;
    ...
    private void handleHeartrate(byte[] value) {
        if (value.length == 2 && value[0] == 0) {
            ...
            //TODO call
            final JSONObject jsonObject = getRateJson(hrValue);
            if(jsonObject != null){
                getRateService().rate(getRateJson(hrValue)).enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        Log.d("Rate", "Json " + jsonObject.toString());
